...
## Phase 8: COBOL-To-Rust Compiler Spine

The newest layer starts the transition from record tooling to source conversion.
It deliberately keeps the existing forensic CLI stable while adding a separate
`cobol2rust` binary and compiler crates for source normalization, syntax,
semantic diagnostics, IR, runtime support, and Rust backend generation.

Key outcomes:

- `cobol2rust convert` accepts COBOL source, copybook directories, dialect, and
  source-format settings.
- The converter emits a Rust project with a vendored `cobol-runtime`.
- Unsupported COBOL, unresolved symbols, and unsupported COPY replacement are
  blocked with `migration-report.json`; they are not silently approximated.
- A hostile-input preflight scanner blocks file sections, hard Data Division
  clauses, complex PERFORM forms, and unsupported Procedure verbs before Rust
  emission.
- `subprojects/cobol-packed-deeper` preserves the earlier codec/formal-methods
  package independently instead of fusing it into the main crate.

## Remaining Boundary

The current codebase is now an advanced record engine plus an early
COBOL-to-Rust compiler spine, not a full dialect-complete COBOL compiler. Full
Procedure Division lowering, EXEC SQL/CICS, unsupported SORT/MERGE shapes,
nested programs, ALTER, dynamic CALL, report writer, production-scale fuzz
campaigns, and real Kani proof execution remain follow-up engineering phases.
