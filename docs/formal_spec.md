...
Strict encode
- strict_encode(value) = encode(value) when scale normalization would not drop
  non-zero fractional digits
- strict_encode(value) returns a precision-loss error when scale normalization
  would drop non-zero fractional digits
...
- Public encode APIs must return typed length/sign/precision errors instead of
