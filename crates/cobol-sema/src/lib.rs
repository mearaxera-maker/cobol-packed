use cobol_ir::{
    BasicBlockIr, ByteRangeIr, CallIr, CallTargetIr, ClassTestIr, CloseFileIr, CobolDialect,
    ConditionAnalysisIr, ConditionIr, ConditionNameIr, ConditionOperandIr, ConditionStatusIr,
    ConditionValueIr, ControlFlowEdgeIr, ControlFlowEdgeKindIr, ControlFlowIr, ControlTransferIr,
    DataItemIr, DataRefIr, DeclarativeIr, DeclarativeTriggerIr, DeleteFileIr, Diagnostic,
    DialectProfileIr, EvaluateAnalysisIr, EvaluateArmIr, EvaluateIr, EvaluatePatternIr,
    EvaluateSubjectIr, FigurativeConstantIr, FileIr, FileKindIr, FileOpenModeIr, FunctionOperandIr,
    GoToIr, IndexItemIr, IndexRepresentationIr, InspectConvertingIr, InspectLikeIr,
    InspectReplacingIr, InspectTallyIr, LinkageParamIr, LinkageSignatureIr, NextSentenceTargetIr,
    OccursIr, OccursKeyDirectionIr, OccursKeyIr, OdoDescriptorIr, OdoTemplateIr, OpenFileIr,
    OperandIr, ParagraphFlowIr, ParagraphIr, PerformIr, PerformVaryingIr, PicCategoryIr, PicIr,
    ProcedureCfgIr, ProcedureRangeIr, ProcedureSentenceIr, ProgramIr, ProgramUnitIr, ReadFileIr,
    RedefinesIr, ReferenceModifierIr, ReferenceResolutionIr, ReferenceResolutionStatusIr,
    ReferenceRoleIr, RelOpIr, ReleaseSortRecordIr, RenamesIr, RerunIr, ResolvedDataRefIr,
    ReturnSortRecordIr, RewriteFileIr, SameRecordAreaIr, SearchAllIr, SearchAllKeyIr, SearchIr,
    SearchWhenIr, SemanticModelIr, SetIndexOperationIr, Severity, SignTestIr, SortDirectionIr,
    SortKeyIr, SortProcedureIr, SourceSpan, StatementBranchIr, StatementIr, StatementPathElementIr,
    StorageAreaIr, StorageBindingIr, StorageCellIr, StorageItemIr, StoragePlanIr,
    StringDelimiterIr, StringOpIr, StringPieceIr, SubscriptExprIr, UnstringOpIr, UnstringTargetIr,
...
pub fn analyze_with_catalog(
    ...
    let (data_items, mut storage) =
        analyze_data_items(ast.data_items, platform_profile, &mut diagnostics);
...
fn ast_data_category(item: &DataDeclAst) -> ValueCategoryIr {
    ...
    if let Some(usage) = usage {
        match usage {
            ...
            "COMP" | "COMP-4" | "COMP-5" | "BINARY" => return ValueCategoryIr::Binary,
    ...
    } else if picture.contains('9') || picture.contains('S') {
...
fn resolved_data_ref(
    ...
    match data_index.resolve_ref(&data_ref) {
        DataResolution::Resolved(item) => {
            ...
            in_redefines = item.redefines.is_some() || data_index.is_redefined_base(item);
        }
        DataResolution::Condition(condition) => {
            ...
            if let DataResolution::Resolved(parent) = data_index.resolve(&condition.parent) {
                ...
                in_redefines = parent.redefines.is_some() || data_index.is_redefined_base(parent);
            }
        }
        ...
        DataResolution::Resolved(item) => {
            ...
            if has_dynamic_occurs_context && unsupported_occurs_context {
                diagnostics.push(Diagnostic::error(
                    ...
                    format!(
                        "field {} is inside OCCURS DEPENDING ON storage; only checked condition/evaluate reads are enabled",
                        item.qualified_name
                    ),
                    paragraph.span.clone(),
                ));
            ...
            if (item.redefines.is_some() || data_index.is_redefined_base(item))
...
fn validate_statement_reachability(paragraphs: &[ParagraphIr], diagnostics: &mut Vec<Diagnostic>) {
    for paragraph in paragraphs {
        ...
        for (statement_idx, statement) in paragraph.statements.iter().enumerate() {
            ...
            if statement_is_terminal_transfer(statement) {
...
fn statement_is_terminal_transfer(statement: &StatementIr) -> bool {
...
fn move_corresponding_item_is_unsupported(
    ...
    item.redefines.is_some()
        || data_index.is_redefined_base(item)
...
fn analyze_data_items(
    ...
    for ast_item in ast_items {
        ...
        if ast_item.level == 88 {
            if let Some((_, parent_idx)) = level_stack.last().copied() {
                let values = extract_values_from_clause_ast(&ast_item.clause_ast)
                    .unwrap_or_else(|| extract_values(&ast_item.clauses));
                let value_set = extract_condition_value_set_from_clause_ast(&ast_item.clause_ast)
    ...
    let storage_cells = build_storage_cell_ir(&storage_items);
    let storage_bindings = build_storage_binding_ir(&storage_items, &conditions, &renames);
    let odo_templates = build_odo_template_ir(&storage_items);
    (
        data_items,
        StoragePlanIr {
            record_length,
...
fn build_storage_cell_ir(items: &[StorageItemIr]) -> Vec<StorageCellIr> {
    items
        .iter()
        .filter(|item| {
            item.addressable
        ...
        .map(|item| StorageCellIr {
            ...
            initial_bytes: storage_template_bytes(item, item.byte_len),
        })
        .collect()
}
...
fn build_odo_template_ir(items: &[StorageItemIr]) -> Vec<OdoTemplateIr> {
    items
        .iter()
        .filter_map(|item| {
            ...
            let fields = if item.value_category == ValueCategoryIr::Group {
                ...
                children
                    ...
                    .map(|child| {
                        (
                            ...
                            storage_template_bytes(child, child.byte_len),
                        )
                    })
                    .collect()
            } else {
                vec![(
                    ...
                    storage_template_bytes(item, item.byte_len / occurs.max.max(1)),
                )]
            };
            Some(OdoTemplateIr {
                table: item.qualified_name.clone(),
...
fn storage_template_bytes(item: &StorageItemIr, len: usize) -> Vec<u8> {
...
fn lower_data_item(
    ...
    let value_category = value_category_for(&usage, picture_ast.as_ref());
    let value = extract_initial_value_from_clause_ast(&item.clause_ast)
        .or_else(|| extract_value(&item.clauses));
    if value_category == ValueCategoryIr::PackedDecimal {
    ...
    paragraphs
        ...
        .map(|paragraph| {
            ...
            let sentences = paragraph
                .sentences
                .into_iter()
                .map(|sentence| ProcedureSentenceIr {
                    statements: sentence
                        .statements
                        .into_iter()
...
fn lower_statement(kind: StatementKindAst, raw: String) -> StatementIr {
    match kind {
        ...
        StatementKindAst::NextSentence => StatementIr::NextSentence,
        ...
        StatementKindAst::Call { target, using } => {
            ...
            {
                CallTargetIr::Literal(target.trim_matches('"').trim_matches('\'').to_string())
            } else {
                CallTargetIr::Identifier(parse_data_ref(&target))
            };
...
fn statement_transfer(
    ...
    statements.iter().find_map(|statement| match statement {
        StatementIr::NextSentence => Some(ControlTransferIr::NextSentence {
...
fn collect_next_sentence_targets_at_path(
    ...
    for (idx, statement) in statements.iter().enumerate() {
        ...
        match statement {
            StatementIr::NextSentence => out.push(NextSentenceTargetIr {
...
fn odo_counter_resolves_to_numeric(occurs: &OccursIr, data_index: &DataReferenceIndex<'_>) -> bool {
    ...
    match data_index.resolve_ref(&DataRefIr::simple(depending_on.clone())) {
        ...
        DataResolution::Special { category, .. } => category_is_numeric(category),
        DataResolution::Missing | DataResolution::Ambiguous(_) | DataResolution::Condition(_) => {
            false
        }
    }
}
...
fn validate_odo_counters(
    ...
    for item in data_items {
        ...
        match data_index.resolve_ref(&DataRefIr::simple(depending_on.clone())) {
            ...
            DataResolution::Special { name, category, byte_len } => diagnostics.push(Diagnostic::error(
                "E_INVALID_ODO_DEPENDING_ON",
                format!(
                    "OCCURS DEPENDING ON {} for {} resolved to special register {} ({:?}, {} bytes)",
...
fn data_references(statement: &StatementIr) -> Vec<(DataRefIr, ReferenceRoleIr)> {
    ...
    match statement {
        ...
        StatementIr::Search(search) => {
            ...
            for when in &search.whens {
                references.extend(condition_references(&when.condition));
        ...
        StatementIr::SearchAll(search) => {
            references.extend(condition_references(&search.key_condition));
            for statement in &search.at_end {
                references.extend(data_references(statement));
            }
    ...
    while let Some(open) = base.find('(') {
        if let Some(close) = base[open + 1..].find(')').map(|idx| idx + open + 1) {
            ...
            if let Some((start, length)) = inner.split_once(':') {
                ...
                subscripts.extend(inner.split(',').map(|part| part.trim().to_string()));
            }
...
fn extract_value(clauses: &str) -> Option<String> {
    ...
    tokens
        .get(value_idx + 1)
        .map(|value| normalize_value_literal(value))
}
...
fn extract_values(clauses: &str) -> Vec<String> {
    let tokens = split_clause_tokens(clauses);
    ...
    tokens
        .iter()
        .skip(value_idx + 1)
        .filter(|part| !part.eq_ignore_ascii_case("THRU") && !part.eq_ignore_ascii_case("THROUGH"))
        .map(|part| normalize_value_literal(part))
        .collect()
}

fn extract_values_from_clause_ast(clauses: &[DataClauseAst]) -> Option<Vec<String>> {
    let value_set = extract_condition_value_set_from_clause_ast(clauses)?;
    Some(
        value_set
            .iter()
...
fn extract_condition_value_set(clauses: &str) -> Vec<ConditionValueIr> {
    ...
    while idx < tokens.len() {
        ...
        let start = normalize_value_literal(token);
        if start.is_empty() {
            idx += 1;
            continue;
        }
        ...
        if tokens
            ...
            .unwrap_or(false)
        {
            if let Some(end) = tokens.get(idx + 2) {
...
"#;
        ...
        assert!(ir.diagnostics.iter().any(|diagnostic| {
            diagnostic.code == "E_UNSUPPORTED_REDEFINES_OVERLAY"
                && diagnostic.message.contains("REC.OVER_GRP.OVER_C")
        }));
    }
