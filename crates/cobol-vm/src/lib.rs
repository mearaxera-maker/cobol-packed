use cobol_dialect::{CollatingSequence, DialectName, DialectProfile, InvalidNumericPolicy};
use cobol_record::{
    decode_binary_integer, decode_packed_decimal, encode_packed_decimal, DecodedValue, Endian,
};
...
impl VmRuntime {
        ...
        for (name, file) in &mut self.files.files {
            ...
            *handle = None;
            ...
            let mut opened = match mode {
                VmOpenMode::Input => OpenOptions::new().read(true).open(&*path),
                VmOpenMode::Io => OpenOptions::new().read(true).write(true).open(&*path),
                VmOpenMode::Output => OpenOptions::new().write(true).create(true).open(&*path),
                VmOpenMode::Extend => OpenOptions::new().append(true).create(true).open(&*path),
            }
    ...
    pub fn index_occurrence(&self, name: &str) -> Result<usize, VmError> {
        ...
        self.indexes
            ...
            .ok_or_else(|| VmError::UnsupportedIndex { name })
    }
    ...
    fn compute_op(&mut self, target: &VmAccessPath, expr: &VmExpr) -> Result<bool, VmError> {
    ...
    fn read_storage_descriptor_bytes(&self, path: &VmAccessPath) -> Result<Vec<u8>, VmError> {
        ...
        match descriptor {
            ...
            VmBinding::OccursCell { program, item } => {
                let occurrence = self.storage_pool_occurrence(path)?;
                self.storage_pool
                    .bytes(&StorageKey::occurrence(program, item, vec![occurrence]))
                    .map(Vec::from)
            }
    ...
    fn storage_pool_occurrence(&self, path: &VmAccessPath) -> Result<usize, VmError> {
        if path.subscripts.len() != 1 {
            return Err(VmError::UnsupportedOperand {
                message: format!(
                    "StoragePool ODO access for {} requires exactly one subscript in this pass",
                    path.target
                ),
            });
        }
        let subscript = &path.subscripts[0];
        let value = decimal_to_i128(to_decimal(&self.eval_expr(&subscript.expr)?)?)?;
        let active_max = match self.storage_descriptor_for(&path.target) {
            Some(VmBinding::OccursCell { program, item }) => self
                .storage_pool
                .active_count_for_occurs_item(program, item)
                .unwrap_or(subscript.max),
            _ => subscript.max,
        };
        ...
        for op in ops {
            match op {
                ...
                VmProcedureOp::Compute {
                    ...
                    let size_error = self.compute_op(target, expr)?;
            ...
            VmExpr::Access(path) => {
                ...
                for subscript in &mut path.subscripts {
                    if let Some(index_name) = subscript.index_name.take() {
                        ...
                        subscript.expr = Box::new(VmExpr::Number(occurrence.to_string()));
                    } else {
                        subscript.expr = Box::new(self.materialize_expr(&subscript.expr)?);
                    }
                }
...
impl VmProgram {
    ...
    pub fn eval_access_path(
        ...
        if let Some(condition_name) = &path.condition_name {
            ...
            return Ok(VmEvaluatedValue {
                value: VmValue::Bool(self.eval_condition_name(bytes, condition_name)?),
                category: VmCategory::Unsupported,
                byte_len: 1,
            });
        }
    ...
    fn decode_field_value(
        ...
        let value = match category {
            ...
            VmCategory::Float | VmCategory::Unsupported => VmValue::Bytes(field_bytes.to_vec()),
        };
        Ok(VmEvaluatedValue {
            value,
            category,
        ...
        if let VmExpr::Access(path) = operand {
            ...
            return Ok(match class {
                VmClassTest::Numeric => match field.category {
                    VmCategory::NumericDisplay => display_bytes_numeric(field_bytes),
                    VmCategory::PackedDecimal => self.decode_packed(field, field_bytes).is_ok(),
                    VmCategory::Binary | VmCategory::NativeBinary => true,
                    _ => false,
                },
                VmClassTest::Alphabetic
...
fn display_bytes_numeric(bytes: &[u8]) -> bool {
    ...
    trimmed
        .chars()
        .enumerate()
        .all(|(idx, ch)| ch.is_ascii_digit() || (idx == 0 && matches!(ch, '+' | '-')))
}
...
mod tests {
    ...
    fn rerun_checkpoint_snapshot_restores_storage_and_file_cursor_from_tape() {
        ...
        let _ = runtime.execute_procedure(&procedure).unwrap();
