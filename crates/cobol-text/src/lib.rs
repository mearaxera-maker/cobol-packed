...
pub fn strip_inline_comment_outside_literals(text: &str) -> &str {
    let mut chars = literal_aware_char_indices(text).peekable();
    ...
    while let Some(item) = chars.next() {
        ...
        if item.ch == '='
            && chars
                .peek()
                .is_some_and(|next| !next.inside_literal && next.ch == '=')
        {
            chars.next();
            in_pseudo_text = !in_pseudo_text;
            continue;
        }
        if in_pseudo_text || item.ch != '*' {
            continue;
        }
        if chars
            .peek()
...
pub fn is_period_outside_literals(text: &str, period_idx: usize) -> bool {
    ...
    literal_aware_char_indices(text)
        .find(|item| item.byte_idx == period_idx)
        .is_some_and(|item| !item.inside_literal)
}
