...
def test_run_oracle_suite_captures_rust_oracle_summary(tmp_path: Path) -> None:
    calls: list[tuple[list[str], Path | None]] = []

    def runner(command: list[str], cwd: Path | None = None) -> subprocess.CompletedProcess[str]:
        calls.append((command, cwd))
        return subprocess.CompletedProcess(command, 0, "oracle ok\n", "")

    result = run_oracle_suite(tmp_path, runner=runner)

    assert calls == [
        (
            ["cargo", "test", "--features", "converter", "--test", "oracle_gnucobol"],
            tmp_path,
        )
    ]
    assert result == {
