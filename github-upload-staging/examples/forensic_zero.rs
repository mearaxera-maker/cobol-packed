...
use cobol_packed::{
    from_packed_lossless, to_packed_lossless, to_packed_with_sign, PackedConfig, SignMode,
};
...
fn main() {
    ...
    println!(
        "Lossless decode : value = {}, sign_nibble = 0x{:X}",
        lossless.value, lossless.sign_nibble
    );

    let lossless_re = to_packed_lossless(&lossless, &cfg).unwrap();
    println!("Lossless re-encode: {}", hex(&lossless_re));
    assert_eq!(
        lossless_re, neg_zero_bytes,
        "byte-for-byte identity required"
    );
    ...
    for &nibble in &[0x0A, 0x0B, 0x0E] {
        ...
        println!(
            "  nibble 0x{nibble:X} → {:?}",
            result.map(|l| (l.value, l.sign_nibble))
        );
    }
