...
//!   cargo kani --harness proof_scalar_simd_agree
//!   cargo kani --harness proof_lossless_roundtrip
//!
//! Requires: `cargo install --locked kani-verifier && cargo kani setup`

use cobol_packed::{
    from_packed, from_packed_lossless, from_packed_scalar,
    to_packed_lossless,
    PackedConfig, SignMode,
};

/// Verify: no public decode call panics for 2-byte inputs under any config.
#[cfg(kani)]
#[kani::proof]
#[kani::unwind(16)]
fn proof_no_panic() {
