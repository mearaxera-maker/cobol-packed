...
return Err(PackedError::ScaleExceedsTotalDigits { scale, total_digits });
}
if scale > 28 {
return Err(PackedError::ScaleTooLargeForDecimal(scale));
}
Ok(Self { total_digits, scale, is_signed })
}
...
/// Total digits.
#[inline]
pub const fn total_digits(&self) -> u8 { self.total_digits }
/// Scale.
#[inline]
pub const fn scale(&self) -> u8 { self.scale }
/// Signedness.
#[inline]
pub const fn is_signed(&self) -> bool { self.is_signed }

/// Packed byte length for this schema.
#[inline]
pub const fn byte_len(&self) -> usize { expected_len(self.total_digits) }
...
pub const LEN: usize = expected_len(DIGITS);

const VALID: () = {
assert!(DIGITS >= 1 && DIGITS <= 18, "DIGITS must be 1..=18");
assert!(SCALE <= DIGITS, "SCALE cannot exceed DIGITS");
assert!(SCALE <= 28, "SCALE cannot exceed Decimal max scale 28");
};
...
///
/// The buffer must have length [`Self::LEN`].
...
pub fn encode_array(value: &Decimal) -> Result<[u8; Self::LEN], PackedError> {
...
pub fn encode_with_sign_into(value: &Decimal, sign_nibble: u8, out: &mut [u8]) -> Result<(), PackedError> {
...
pub fn encode_with_sign_array(value: &Decimal, sign_nibble: u8) -> Result<[u8; Self::LEN], PackedError> {
let mut out = [0u8; Self::LEN];
...
pub fn decode_into(bytes: &[u8], sign_mode: SignMode, out: &mut Decimal) -> Result<(), PackedError> {
*out = decode_with_config(bytes, &Self::cfg(), sign_mode)?;
Ok(())
}

/// Decode losslessly using the const-generic configuration.
pub fn decode_lossless(bytes: &[u8], sign_mode: SignMode) -> Result<LosslessDecimal, PackedError> {
decode_lossless_with_config(bytes, &Self::cfg(), sign_mode)
}

/// Decode losslessly into a caller-provided slot.
pub fn decode_lossless_into(bytes: &[u8], sign_mode: SignMode, out: &mut LosslessDecimal) -> Result<(), PackedError> {
...
pub fn encode_lossless_into(lossless: &LosslessDecimal, out: &mut [u8]) -> Result<(), PackedError> {
encode_lossless_into_with_config(lossless, &Self::cfg(), out)
}

/// Lossless re-encode into a fixed-size stack array.
pub fn encode_lossless_array(lossless: &LosslessDecimal) -> Result<[u8; Self::LEN], PackedError> {
let mut out = [0u8; Self::LEN];
...
InvalidByteLength { expected: usize, actual: usize, total_digits: u8 },
...
ScaleExceedsTotalDigits { scale: u8, total_digits: u8 },

/// Decimal scale is too large.
#[error("scale ({0}) exceeds Decimal max scale 28")]
...
InvalidDigitNibble { position: usize, nibble: u8 },
...
ArithmeticOverflow { target_scale: u32, current_scale: u32 },
...
InvalidSignOverride { nibble: u8, is_signed: bool, is_negative: bool },
...
#[cfg(feature = "simd")]
{
return simd::expand_nibbles(bytes) == scalar_expand_nibbles(bytes);
}
...
pow10(total_digits as u32).expect("total_digits <= 18") - 1
}
...
Ok((if mant < 0 { -mant } else { mant }, mant < 0 || value.is_sign_negative()))
}
...
abs_mant.checked_mul(factor).ok_or(PackedError::ArithmeticOverflow {
target_scale,
current_scale,
})?
} else {
...
return Err(PackedError::Overflow { max_digits: cfg.total_digits });
}
Ok((scaled, is_negative))
}

fn validate_sign_override(sign: u8, is_signed: bool, is_negative: bool, is_zero: bool) -> Result<(), PackedError> {
if is_signed {
if is_zero {
if matches!(sign, 0xA | 0xB | 0xC | 0xD | 0xE | 0xF) {
return Ok(());
}
} else if (!is_negative && matches!(sign, 0xA | 0xC | 0xE | 0xF)) || (is_negative && matches!(sign, 0xB | 0xD)) {
return Ok(());
}
} else if !is_negative && matches!(sign, 0xA | 0xC | 0xE | 0xF) {
return Ok(());
}
Err(PackedError::InvalidSignOverride { nibble: sign, is_signed, is_negative })
}

fn pack_integer(mut value: i128, buf: &mut [u8], sign: u8, total_digits: u8) {
let td = total_digits as usize;
let byte_len = expected_len(total_digits);
assert_eq!(buf.len(), byte_len, "pack_integer called with incorrect output buffer length");
...
simd::expand_nibbles(bytes)
}

fn decode_scalar_core(bytes: &[u8], cfg: &PackedConfig, sign_mode: SignMode) -> Result<(i128, u8), PackedError> {
let expected = expected_len(cfg.total_digits);
if bytes.len() != expected {
return Err(PackedError::InvalidByteLength { expected, actual: bytes.len(), total_digits: cfg.total_digits });
}
...
let nib = nibbles.next().ok_or(PackedError::InvalidByteLength { expected, actual: bytes.len(), total_digits: cfg.total_digits })?;
if nib > 9 {
return Err(PackedError::InvalidDigitNibble { position: i, nibble: nib });
}
accum = accum * 10 + i128::from(nib);
}

let sign_nibble = nibbles.next().ok_or(PackedError::InvalidByteLength { expected, actual: bytes.len(), total_digits: cfg.total_digits })?;
let is_negative = match sign_mode {
SignMode::Pfd => match (sign_nibble, cfg.is_signed) {
(0xC, _) => false,
(0xD, true) => true,
(0xD, false) => return Err(PackedError::NegativeInUnsigned { nibble: sign_nibble }),
(0xF, false) => false,
_ => return Err(PackedError::InvalidSignNibble { nibble: sign_nibble, sign_mode, is_signed: cfg.is_signed }),
},
SignMode::Nopfd => match sign_nibble {
0xA | 0xC | 0xE | 0xF => false,
0xB | 0xD => {
if !cfg.is_signed {
return Err(PackedError::NegativeInUnsigned { nibble: sign_nibble });
}
true
}
_ => return Err(PackedError::InvalidSignNibble { nibble: sign_nibble, sign_mode, is_signed: cfg.is_signed }),
...
fn decode_core(bytes: &[u8], cfg: &PackedConfig, sign_mode: SignMode) -> Result<(i128, u8), PackedError> {
#[cfg(feature = "simd")]
{
assert!(simd_matches_scalar(bytes), "SIMD nibble expansion diverged from scalar reference");
}
decode_scalar_core(bytes, cfg, sign_mode)
}

fn decode_with_config(bytes: &[u8], cfg: &PackedConfig, sign_mode: SignMode) -> Result<Decimal, PackedError> {
let (signed, _) = decode_core(bytes, cfg, sign_mode)?;
Ok(Decimal::from_i128_with_scale(signed, cfg.scale as u32))
}

fn decode_lossless_with_config(bytes: &[u8], cfg: &PackedConfig, sign_mode: SignMode) -> Result<LosslessDecimal, PackedError> {
let (signed, sign_nibble) = decode_core(bytes, cfg, sign_mode)?;
Ok(LosslessDecimal { value: Decimal::from_i128_with_scale(signed, cfg.scale as u32), sign_nibble })
}

fn encode_into_with_policy(value: &Decimal, cfg: &PackedConfig, zero_sign: ZeroSignPolicy, out: &mut [u8]) -> Result<(), PackedError> {
let expected = expected_len(cfg.total_digits);
if out.len() != expected {
return Err(PackedError::InvalidByteLength { expected, actual: out.len(), total_digits: cfg.total_digits });
}
...
if is_zero {
if cfg.is_signed { if is_negative { 0x0D } else { 0x0C } } else { 0x0F }
} else {
...
fn encode_into_with_config(value: &Decimal, cfg: &PackedConfig, out: &mut [u8]) -> Result<(), PackedError> {
encode_into_with_policy(value, cfg, ZeroSignPolicy::Canonical, out)
}

fn encode_with_sign_into_policy(value: &Decimal, cfg: &PackedConfig, sign_nibble: u8, zero_sign: ZeroSignPolicy, out: &mut [u8]) -> Result<(), PackedError> {
let expected = expected_len(cfg.total_digits);
if out.len() != expected {
return Err(PackedError::InvalidByteLength { expected, actual: out.len(), total_digits: cfg.total_digits });
}
...
fn encode_with_sign_into_config(value: &Decimal, cfg: &PackedConfig, sign_nibble: u8, out: &mut [u8]) -> Result<(), PackedError> {
encode_with_sign_into_policy(value, cfg, sign_nibble, ZeroSignPolicy::Preserve, out)
}

fn encode_with_policy(value: &Decimal, cfg: &PackedConfig, zero_sign: ZeroSignPolicy) -> Result<Vec<u8>, PackedError> {
let (scaled, is_negative) = scale_mantissa(value, cfg)?;
if is_negative && !cfg.is_signed {
return Err(PackedError::NegativeUnsigned);
}
...
fn encode_with_sign_policy(value: &Decimal, cfg: &PackedConfig, sign_nibble: u8, zero_sign: ZeroSignPolicy) -> Result<Vec<u8>, PackedError> {
let (scaled, is_negative) = scale_mantissa(value, cfg)?;
if is_negative && !cfg.is_signed {
return Err(PackedError::NegativeUnsigned);
}
...
fn encode_with_sign_config(value: &Decimal, cfg: &PackedConfig, sign_nibble: u8) -> Result<Vec<u8>, PackedError> {
encode_with_sign_policy(value, cfg, sign_nibble, ZeroSignPolicy::Preserve)
}

fn encode_lossless_with_policy(lossless: &LosslessDecimal, cfg: &PackedConfig, zero_sign: ZeroSignPolicy) -> Result<Vec<u8>, PackedError> {
...
fn encode_lossless_with_config(lossless: &LosslessDecimal, cfg: &PackedConfig) -> Result<Vec<u8>, PackedError> {
encode_lossless_with_policy(lossless, cfg, ZeroSignPolicy::Preserve)
}

fn encode_lossless_into_with_policy(lossless: &LosslessDecimal, cfg: &PackedConfig, zero_sign: ZeroSignPolicy, out: &mut [u8]) -> Result<(), PackedError> {
let expected = expected_len(cfg.total_digits);
if out.len() != expected {
return Err(PackedError::InvalidByteLength { expected, actual: out.len(), total_digits: cfg.total_digits });
}
...
fn encode_lossless_into_with_config(lossless: &LosslessDecimal, cfg: &PackedConfig, out: &mut [u8]) -> Result<(), PackedError> {
...
pub fn from_packed_scalar(bytes: &[u8], cfg: &PackedConfig, sign_mode: SignMode) -> Result<Decimal, PackedError> {
decode_scalar_core(bytes, cfg, sign_mode).map(|(signed, _)| Decimal::from_i128_with_scale(signed, cfg.scale as u32))
}

/// Decode a packed-decimal byte slice using the scalar reference decoder into a caller-provided slot.
pub fn from_packed_scalar_into(bytes: &[u8], cfg: &PackedConfig, sign_mode: SignMode, out: &mut Decimal) -> Result<(), PackedError> {
*out = from_packed_scalar(bytes, cfg, sign_mode)?;
Ok(())
}

/// Decode a packed-decimal byte slice with an explicit policy object.
pub fn from_packed_with_policy(bytes: &[u8], cfg: &PackedConfig, policy: PackedPolicy) -> Result<Decimal, PackedError> {
decode_with_config(bytes, cfg, policy.sign_mode)
}

/// Decode a packed-decimal byte slice.
pub fn from_packed(bytes: &[u8], cfg: &PackedConfig, sign_mode: SignMode) -> Result<Decimal, PackedError> {
decode_with_config(bytes, cfg, sign_mode)
}

/// Decode a packed-decimal byte slice into a caller-provided `Decimal` slot.
pub fn from_packed_into(bytes: &[u8], cfg: &PackedConfig, sign_mode: SignMode, out: &mut Decimal) -> Result<(), PackedError> {
*out = decode_with_config(bytes, cfg, sign_mode)?;
Ok(())
}

/// Decode a packed-decimal byte slice, preserving the original sign nibble.
pub fn from_packed_lossless(bytes: &[u8], cfg: &PackedConfig, sign_mode: SignMode) -> Result<LosslessDecimal, PackedError> {
decode_lossless_with_config(bytes, cfg, sign_mode)
}

/// Decode a packed-decimal byte slice losslessly into a caller-provided slot.
pub fn from_packed_lossless_into(bytes: &[u8], cfg: &PackedConfig, sign_mode: SignMode, out: &mut LosslessDecimal) -> Result<(), PackedError> {
...
pub fn from_packed_lossless_with_policy(bytes: &[u8], cfg: &PackedConfig, policy: PackedPolicy) -> Result<LosslessDecimal, PackedError> {
...
pub fn to_packed_with_policy(value: &Decimal, cfg: &PackedConfig, policy: PackedPolicy) -> Result<Vec<u8>, PackedError> {
encode_with_policy(value, cfg, policy.zero_sign)
}

/// Encode a decimal into a caller-provided stack buffer.
pub fn to_packed_into(value: &Decimal, cfg: &PackedConfig, out: &mut [u8]) -> Result<(), PackedError> {
encode_into_with_config(value, cfg, out)
}

/// Encode a decimal into a caller-provided stack buffer under an explicit policy.
pub fn to_packed_into_with_policy(value: &Decimal, cfg: &PackedConfig, policy: PackedPolicy, out: &mut [u8]) -> Result<(), PackedError> {
encode_into_with_policy(value, cfg, policy.zero_sign, out)
}

/// Encode with an explicit sign nibble.
pub fn to_packed_with_sign(value: &Decimal, cfg: &PackedConfig, sign_nibble: u8) -> Result<Vec<u8>, PackedError> {
encode_with_sign_config(value, cfg, sign_nibble)
}

/// Encode with an explicit sign nibble and zero/sign policy.
pub fn to_packed_with_sign_with_policy(value: &Decimal, cfg: &PackedConfig, sign_nibble: u8, policy: PackedPolicy) -> Result<Vec<u8>, PackedError> {
encode_with_sign_policy(value, cfg, sign_nibble, policy.zero_sign)
}

/// Encode with an explicit sign nibble into a caller-provided stack buffer.
pub fn to_packed_with_sign_into(value: &Decimal, cfg: &PackedConfig, sign_nibble: u8, out: &mut [u8]) -> Result<(), PackedError> {
...
pub fn to_packed_with_sign_into_with_policy(value: &Decimal, cfg: &PackedConfig, sign_nibble: u8, policy: PackedPolicy, out: &mut [u8]) -> Result<(), PackedError> {
encode_with_sign_into_policy(value, cfg, sign_nibble, policy.zero_sign, out)
}

/// Lossless re-encode using the original sign nibble.
pub fn to_packed_lossless(lossless: &LosslessDecimal, cfg: &PackedConfig) -> Result<Vec<u8>, PackedError> {
encode_lossless_with_config(lossless, cfg)
}

/// Lossless re-encode under an explicit policy object.
pub fn to_packed_lossless_with_policy(lossless: &LosslessDecimal, cfg: &PackedConfig, policy: PackedPolicy) -> Result<Vec<u8>, PackedError> {
encode_lossless_with_policy(lossless, cfg, policy.zero_sign)
}

/// Lossless re-encode into a caller-provided stack buffer.
pub fn to_packed_lossless_into(lossless: &LosslessDecimal, cfg: &PackedConfig, out: &mut [u8]) -> Result<(), PackedError> {
...
pub fn to_packed_lossless_into_with_policy(lossless: &LosslessDecimal, cfg: &PackedConfig, policy: PackedPolicy, out: &mut [u8]) -> Result<(), PackedError> {
...
fn ds(s: &str) -> Decimal { Decimal::from_str(s).unwrap() }
fn cfg(td: u8, sc: u8, signed: bool) -> PackedConfig { PackedConfig::new(td, sc, signed).unwrap() }
...
let mut lossless_out = LosslessDecimal { value: Decimal::ZERO, sign_nibble: 0 };
...
let c = cfg(4, 2, true);
assert_eq!(from_packed(&[0x01, 0x23, 0x4C], &c, SignMode::Pfd).unwrap(), ds("12.34"));
}
...
let c = cfg(4, 2, true);
assert_eq!(to_packed(&ds("12.345"), &c).unwrap(), vec![0x01, 0x23, 0x4C]);
assert_eq!(to_packed(&ds("-12.345"), &c).unwrap(), vec![0x01, 0x23, 0x4D]);
}
...
let c = cfg(18, 0, true);
assert!(matches!(to_packed(&Decimal::MIN, &c), Err(PackedError::AbsoluteOverflow)));
}
...
assert_eq!(to_packed_with_policy(&zero, &c, canonical).unwrap(), vec![0x00, 0x0C]);
assert_eq!(to_packed_with_policy(&zero, &c, lossless).unwrap(), vec![0x00, 0x0C]);

let loss = from_packed_lossless_with_policy(&[0x00, 0x0D], &c, lossless).unwrap();
assert_eq!(to_packed_lossless_with_policy(&loss, &c, lossless).unwrap(), vec![0x00, 0x0D]);
}
...
let c = cfg(3, 0, false);
let value = d(123);
assert_eq!(to_packed_with_sign(&value, &c, 0x0A).unwrap(), vec![0x12, 0x3A]);
assert_eq!(to_packed_with_sign(&value, &c, 0x0E).unwrap(), vec![0x12, 0x3E]);
...
assert!(simd_matches_scalar(&[0x01, 0x23, 0x4C, 0xAD, 0xEF, 0x00, 0x99, 0x10, 0xFF, 0x7B]));
}
...
pack_integer(0, &mut buf, sign, total_digits);
buf
} else {
