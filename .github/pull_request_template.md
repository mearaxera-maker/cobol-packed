...
- [ ] `cargo audit` passes or the finding is documented
- [ ] `cargo deny check` passes or the policy exception is documented
- [ ] New public items have `///` documentation
- [ ] CHANGELOG.md updated under `[Unreleased]`
- [ ] If touching `simd.rs`: `simd_and_scalar_agree` test passes
- [ ] If touching converter support status: capability matrix regression check passes
- [ ] If touching oracle fixtures: GnuCOBOL version/evidence is captured
- [ ] If touching release/packaging: license, notice, SBOM, and checksum artifacts are preserved
